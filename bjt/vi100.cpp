...
void fit1(){
  ...
  TGraphErrors * graph1 = new TGraphErrors("vi100.txt","%lg %*lg %lg %lg");
  
  graph1->SetTitle("Base 0.1mA"); 
  ...
  graph1->GetYaxis()->SetTitle("Corrente -I_{C} (mA)");
    
  graph1->Draw("APE");


  TF1 *fit = new TF1("f1","pol1",16,20);
  fit->SetParName(0, "a");
  fit->SetParName(1, "b"); 
